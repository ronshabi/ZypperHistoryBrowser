...
DISCARDED_CHARS = ["#", "zypper", "yast"]
try:
    with open(DEFAULT_FILE_LOCATION) as f:
        lines = f.readlines()
        
        # eat lines with discarded chars
        lines = [line for line in lines if not any(c in line for c in DISCARDED_CHARS)]

        # eat lines with discarded words
        lines = [line for line in lines if not any(word.lower() in DISCARDED_CHARS for word in line)]
